...
impl<S: CosmosSigner> PacketEvents for CosmosChain<S> {
    ...
    async fn subscribe_block_events(
        ...
        Option<
            Pin<
                Box<
                    dyn Stream<Item = Result<BlockEvents<tendermint::block::Height, CosmosEvent>>>
                        + Send,
                >,
            >,
        >,
...
fn cosmos_ws_stream(
    ...
    futures::stream::unfold(
        (
            subscription,
            client,
            None::<(tendermint::block::Height, Vec<CosmosEvent>)>,
            flush_timeout,
        ),
        |(mut sub, client, mut pending, flush_timeout)| async move {
            let flush = |pending: (tendermint::block::Height, Vec<CosmosEvent>), state| {
                ...
                match next {
                        ...
                        match &mut pending {
                            ...
                            Some((pending_height, pending_events)) => {
                                ...
                                *pending_events = events;
                                return Some((Ok(flushed), (sub, client, pending, flush_timeout)));
                            }
                            None => {
                    ...
                    Ok(Some(Err(e))) => {
                        if let Some(p) = pending.take() {
                            return flush(p, (sub, client, None, flush_timeout));
                        }
                        return Some((
                            Err(eyre::eyre!("websocket stream error: {e}")),
                            (sub, client, None, flush_timeout),
                        ));
                    }
                    Ok(None) => {
                        return pending
                            .take()
                            .and_then(|p| flush(p, (sub, client, None, flush_timeout)));
                    }
                    Err(_timeout) => {
                        if let Some(p) = pending.take() {
                            return flush(p, (sub, client, None, flush_timeout));
